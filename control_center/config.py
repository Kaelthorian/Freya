...
DEFAULT_CONFIG = {
    ...
    "secret_env": "",
    "workspace_path": "",
    # Optional, validated origin metadata for agents generated by Freya. It
    # carries no authority and remains in immutable task snapshots.
    "provenance": {},
...
def _normalize_provenance(value) -> dict:
    """Validate non-authoritative metadata for Freya-generated agents."""
    if value in (None, {}):
        return {}
    if not isinstance(value, dict):
        raise ValueError("provenance must be an object.")
    allowed = {
        "generated_by_freya", "orchestration_id", "plan_task_id", "attempt",
        "factory_version", "ephemeral",
    }
    unknown = set(value) - allowed
    if unknown:
        raise ValueError("provenance contains unknown fields: " + ", ".join(sorted(unknown)))
    if value.get("generated_by_freya") is not True or value.get("ephemeral") is not True:
        raise ValueError("Freya provenance requires generated_by_freya and ephemeral to be true.")
    result = {"generated_by_freya": True, "ephemeral": True}
    for field in ("orchestration_id", "plan_task_id"):
        item = value.get(field)
        if not isinstance(item, str) or not item.strip() or len(item.strip()) > 200:
            raise ValueError(
                f"provenance.{field} must be non-empty text up to 200 characters."
            )
        result[field] = item.strip()
    for field in ("attempt", "factory_version"):
        item = value.get(field)
...
def normalize_agent(data: dict, existing: dict | None = None, *,
                    allow_provenance: bool = False) -> dict:
    ...
    config["provenance"] = _normalize_provenance(config.get("provenance"))
